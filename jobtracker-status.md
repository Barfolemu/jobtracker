...
**Verified twice**: once via manual `aws lambda invoke` (found 5 distinct jobs from 7 matching emails, 2 correctly deduped as same posting from different saved searches), once via the EventBridge Scheduler firing naturally on its own 60-minute rate schedule (`jobtracker-gmail-scan`) with zero manual intervention — same 5 jobs, clean run, no errors. Confirmed the dedupe design is safe even under an accidental duplicate invocation (AWS CLI retried mid-cold-start during the manual test; the retry correctly saw everything as already-written and wrote nothing twice).
